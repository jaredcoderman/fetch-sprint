...
function SchoolCompetitions() {
  ...
  function handleChangeSchool() {
    // Clear only the school from localStorage, keep other profile info
    const savedProfile = localStorage.getItem('userProfile');
    if (savedProfile) {
      const profile = JSON.parse(savedProfile);
      // Remove only the schoolName, keep firstName, lastName, email, etc.
      const { schoolName, ...profileWithoutSchool } = profile;
      localStorage.setItem('userProfile', JSON.stringify(profileWithoutSchool));
    }
    // Redirect to school selection with changeSchool parameter
    navigate('/profile?changeSchool=true');
  }
  ...
  async function loadSchoolData() {
    try {
      ...
      // If not found, create school entry
      const newSchoolDoc = await addDoc(collection(db, 'schools'), {
        name: schoolName,
        competitionCount: 0,
        participantCount: 0,
  ...
  async function loadCompetitions() {
    try {
      ...
      let competitionsSnapshot = await getDocs(competitionsQuery);
      
      // If no exact match, try case-insensitive search
      if (competitionsSnapshot.size === 0) {
        const allCompetitionsQuery = query(collection(db, 'competitions'));
        const allSnapshot = await getDocs(allCompetitionsQuery);
        
        const matchingComps = allSnapshot.docs.filter(doc => {
          const data = doc.data();
          return data.schoolName && data.schoolName.toLowerCase() === schoolName.toLowerCase();
        });
        
        const comps = matchingComps.map(doc => ({
          id: doc.id,
          ...doc.data()
        }));
  ...
  function handleCreateCompetition() {
    navigate(`/create-competition/${encodeURIComponent(schoolName)}`);
  }
    ...
    <div className="min-h-screen bg-blue-shiny-gradient">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="bg-white text-black rounded-xl shadow-lg p-8 mb-8 text-center border-2 border-yellow-300">
          <h2 className="text-4xl font-bold mb-2 text-black">{schoolName}</h2>
          {schoolData && schoolData.city && schoolData.state && (
            <p className="text-lg text-blue-800 mb-2">{schoolData.city}, {schoolData.state}</p>
          )}
        ...
        {competitions.length === 0 ? (
          <div className="bg-white rounded-xl shadow-lg p-12 text-center">
            ...
            <p className="text-gray-600 mb-8">
              Be the first to create a competition for {schoolName}!
            </p>
            <button
              onClick={handleCreateCompetition}
